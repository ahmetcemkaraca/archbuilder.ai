...
async def get_subscription_info(
    ...
    try:
        ...
        logger.error(f"Failed to get subscription info for user {current_user.id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def check_subscription(
    ...
    """
    try:
        subscription_info = await license_service.check_subscription_status(current_user.id)
        ...
        return {
            ...
            "trial_end": subscription_info.trial_end.isoformat() if subscription_info.trial_end else None
        }
...
async def check_feature_access(
    ...
    try:
        ...
        logger.error(f"Feature access check failed for user {current_user.id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def refresh_subscription(
    ...
    try:
        ...
        logger.error(f"Subscription refresh failed for user {current_user.id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_license_status(
    ...
    try:
        ...
        logger.error(f"Failed to get license status for user {current_user.id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
