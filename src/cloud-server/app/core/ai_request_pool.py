...
class AIRequestPool:
    ...
    def __init__(self, max_concurrent_requests: int = 50):
        ...
        self.user_rate_limits: Dict[str, Dict[str, List[datetime]]] = defaultdict(
            lambda: defaultdict(list)
        )
        self.model_rate_limits: Dict[str, List[datetime]] = defaultdict(list)

        # Cost tracking
        self.user_costs: Dict[str, Dict[str, float]] = defaultdict(
        ...
        while True:
            try:
                ...
                if not await self._check_rate_limits(
                    request.user_id, request.model, request.provider
                ):
    ...
    async def _process_single_request(self, request: AIRequest):
        ...
        try:
            ...
            logger.error(
                "AI request failed", request_id=request.request_id, error=str(e)
            )
    ...
    async def _check_rate_limits(self, user_id: str, model: str, provider: str) -> bool:
        ...
        model_requests = [
            req_time
            for req_time in model_requests
            if now - req_time < timedelta(minutes=1)
        ]
    ...
    async def _update_rate_limits(self, request: AIRequest):
        ...
        self.model_rate_limits[request.model] = [
            req_time
            for req_time in self.model_rate_limits[request.model]
            if req_time > cutoff_time
        ]
    ...
    async def _cleanup_completed_requests(self):
        ...
        while True:
            try:
                ...
                if old_requests:
                    logger.info(
                        "Cleaned up old completed requests", count=len(old_requests)
                    )
    ...
    def get_user_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        user_requests = [
            req for req in self.completed_requests.values() if req.user_id == user_id
        ]
