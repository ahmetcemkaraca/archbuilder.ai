    ...
    def _add_pool_event_listeners(self) -> None:
        ...
        def receive_checkin(dbapi_connection, connection_record):
            if hasattr(connection_record, '_checkout_time'):
                ...
                logger.debug("Connection checked in", checkout_duration=checkout_duration)
        ...
        try:
            ...
            if hasattr(pool, 'size'):
                metrics = PoolMetrics(
                    ...
                    utilization_percentage=(pool.checkedout() / pool.size()) * 100 if pool.size() > 0 else 0,
                    overflow_percentage=(pool.overflow() / pool.size()) * 100 if pool.size() > 0 else 0,
                    avg_connection_time=sum(self._connection_times[-100:]) / min(len(self._connection_times), 100) if self._connection_times else 0.0,
                ...
                if metrics.utilization_percentage > 80:
                    logger.warning("High pool utilization", utilization=metrics.utilization_percentage)
...
def initialize_pool_manager(database_url: str, config: PoolConfig) -> ConnectionPoolManager:
