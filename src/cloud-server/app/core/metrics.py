...
class ArchBuilderMetrics:
    ...
    def record_ai_request(
        ...
        self.ai_request_duration_seconds.labels(model=model, provider=provider).observe(
            duration
        )
        ...
        if confidence > 0:
            self.ai_confidence_score.labels(model=model, provider=provider).observe(
                confidence
            )

    def record_document_upload(
        self, file_type: str, status: str, file_size: int, duration: float
    ):
    ...
    def record_database_query(self, query_type: str, status: str, duration: float):
        ...
        self.database_queries_total.labels(query_type=query_type, status=status).inc()
    ...
    def record_rate_limit_hit(self, endpoint: str, user_tier: str):
        ...
        self.rate_limit_hits_total.labels(endpoint=endpoint, user_tier=user_tier).inc()
        ...
        try:
            ...
            if pool_metrics:
                self.metrics.update_database_connections(
                    pool_metrics.checked_out, pool_metrics.checked_in
                )
