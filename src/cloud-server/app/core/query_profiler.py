...
class QueryProfiler:
    ...
    def _update_query_stats(self, profile: QueryProfile) -> None:
        ...
        if query_key not in self.query_stats:
            self.query_stats[query_key] = {
                ...
                'max_time_ms': 0.0,
                'slow_count': 0,
                'query_text': profile.query_text[:100] + '...' if len(profile.query_text) > 100 else profile.query_text
            }
    ...
    def get_slow_queries(self, limit: int = 50) -> List[QueryProfile]:
        ...
        return sorted(slow_queries, key=lambda x: x.execution_time_ms, reverse=True)[:limit]
    ...
    def get_query_statistics(self) -> Dict[str, Any]:
        ...
        for profile in self.query_profiles:
            ...
            if query_type not in type_stats:
                type_stats[query_type] = {'count': 0, 'avg_time_ms': 0.0, 'max_time_ms': 0.0}
            ...
            type_stats[query_type]['max_time_ms'] = max(
                type_stats[query_type]['max_time_ms'], 
    ...
    def clear_old_profiles(self, hours: int = 24) -> int:
        ...
        logger.info("Cleared old query profiles", cleared_count=cleared_count, hours=hours)
...
class DatabaseOptimizer:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                return DatabaseMetrics(
                    ...
                    slow_queries=len([q for q in self.profiler.query_profiles if q.is_slow]),
                    avg_execution_time_ms=sum(q.execution_time_ms for q in self.profiler.query_profiles) / max(len(self.profiler.query_profiles), 1),
                    max_execution_time_ms=max((q.execution_time_ms for q in self.profiler.query_profiles), default=0.0),
                    ...
                    index_usage_stats={row['indexname']: row['idx_scan'] for row in index_stats}
                )
    ...
    async def optimize_connection_pool(self) -> Dict[str, Any]:
        ...
        if utilization > 0.8:
            ...
            recommendations.append("Low pool utilization - consider decreasing pool size")
        
        if overflow > 0:
            recommendations.append("Pool overflow detected - increase pool size or timeout")
...
async def profile_query(query_name: str):
    ...
    try:
        logger.debug("Query profiling started", query_name=query_name, query_id=query_id)
        yield query_id
    finally:
