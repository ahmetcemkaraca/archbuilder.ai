...
class AuditRetentionManager:
    ...
    def _create_default_policy(self) -> RetentionPolicy:
        ...
        rules = [
            ...
            RetentionRule(
                event_types=[
                    AuditEventType.PROJECT_CREATE,
                    AuditEventType.PROJECT_UPDATE,
    ...
    async def cleanup_expired_events(self) -> Dict[str, Any]:
        ...
        self.audit_events = [
            event for event in self.audit_events if event not in expired_events
        ]
    ...
    async def get_retention_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "events_by_retention": {
                retention: len(events)
                for retention, events in events_by_retention.items()
            },
            "retention_rules": [
                {
                    "event_types": [
                        event_type.value for event_type in rule.event_types
                    ],
                    "data_classifications": [
                        classification.value
                        for classification in rule.data_classifications
                    ],
    ...
    async def anonymize_old_events(
        self, anonymization_threshold_days: int = 365
    ) -> Dict[str, Any]:
        """Anonymize old events while preserving audit trail"""
        threshold_date = datetime.utcnow() - timedelta(
            days=anonymization_threshold_days
        )

        old_events = [
            event for event in self.audit_events if event.timestamp < threshold_date
        ]
        ...
        for event in old_events:
            ...
            event.user_id = (
                f"ANONYMIZED_{hashlib.md5(event.user_id.encode()).hexdigest()[:8]}"
            )
...
class ComplianceReporter:
    ...
    async def generate_gdpr_report(self) -> Dict[str, Any]:
        ...
        personal_data_events = [
            event
            for event in self.retention_manager.audit_events
        ...
        return {
            ...
            "compliance_rate": (
                compliant_events / len(personal_data_events)
    ...
    async def generate_sox_report(self) -> Dict[str, Any]:
        ...
        financial_events = [
            ...
            for event in self.retention_manager.audit_events
            if event.event_type
            in [AuditEventType.PAYMENT_PROCESSED, AuditEventType.SUBSCRIPTION_CHANGE]
        ]
        ...
        return {
            ...
            "compliance_rate": (
                long_term_retained / len(financial_events) if financial_events else 1.0
            ),
