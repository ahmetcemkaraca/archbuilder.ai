...
class FileAbuseDetector:
    ...
    async def _analyze_file_content(
        ...
        try:
            ...
            logger.warning(
                "Error analyzing file content", error=str(e), filename=metadata.filename
            )
    ...
    def _determine_risk_level(
        ...
        # High risk factors
        high_types = {AbuseType.EXECUTABLE_CONTENT, AbuseType.SUSPICIOUS_FORMAT}

        if (
            any(abuse_type in high_types for abuse_type in abuse_types)
            or confidence > 0.7
        ):
            return "high"

        # Medium risk factors
        medium_types = {AbuseType.OVERSIZED_FILE, AbuseType.REPEATED_UPLOADS}

        if (
            any(abuse_type in medium_types for abuse_type in abuse_types)
            or confidence > 0.4
        ):
            return "medium"
    ...
    async def scan_upload_directory(
        ...
        try:
            ...
            logger.error(
                "Error scanning directory", error=str(e), directory=directory_path
            )
...
class FileAbuseMonitor:
    ...
    async def get_user_upload_stats(
        ...
        time_span = max(upload.upload_timestamp for upload in uploads) - min(
            upload.upload_timestamp for upload in uploads
        )
...
async def analyze_uploaded_file(
    file_path: str, metadata: FileMetadata
) -> AbuseDetectionResult:
