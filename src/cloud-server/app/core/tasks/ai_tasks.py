...
class AIInferenceTask(ArchBuilderTask):
    ...
    def run(self, task_request_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_key = (
                f"ai_response:{task_request.correlation_id}:{model_config['model']}"
            )
    ...
    def _process_ai_inference(
        ...
        validation_result = validator.validate_response(
            response=response, correlation_id=task_request.correlation_id
        )
...
def validate_ai_responses(self, correlation_ids: List[str]) -> Dict[str, Any]:
    ...
    logger.info(
        "Starting AI response validation", correlation_count=len(correlation_ids)
    )
    ...
    for correlation_id in correlation_ids:
        try:
            ...
            cached_response = asyncio.run(
                cache_service.get_ai_response(correlation_id, "unknown")
            )

            if cached_response:
                validation_result = validator.validate_response(
                    response=cached_response, correlation_id=correlation_id
                )
