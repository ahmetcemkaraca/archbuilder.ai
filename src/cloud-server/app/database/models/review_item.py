...
class ReviewItem(Base):
    ...
    status = Column(
        String(20), nullable=False, default="pending", comment="TR: Review durumu"
    )
    priority = Column(
        String(20), nullable=False, default="medium", comment="TR: Review önceliği"
    )

    # TR: Atama bilgileri
    assigned_to = Column(String(100), nullable=True, comment="TR: Atanan reviewer")
    assigned_at = Column(
        DateTime(timezone=True), nullable=True, comment="TR: Atama tarihi"
    )
    ...
    reviewed_at = Column(
        DateTime(timezone=True), nullable=True, comment="TR: Review tarihi"
    )
