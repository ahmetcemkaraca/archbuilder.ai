...
from ..schemas.layout_schemas import (
    LayoutGenerationRequest, LayoutResult, ReviewFeedback,
    ValidationResult, LayoutStatus
)
...
from ..core.dependencies import (
    ...
    get_ai_service, get_model_selector
)
...
async def generate_layout(
    ...
    try:
        ...
        logger.info(
            ...
            requires_review=result.validation_result.requires_human_review if result.validation_result else False
        )
        
        # Add background task for follow-up processing if needed
        if result.status == LayoutStatus.PROCESSING:
            background_tasks.add_task(
                _monitor_generation_progress,
                result.layout_id,
                current_user.get("id")
            )
...
async def validate_layout(
    ...
    # Re-run validation
    validator = ComprehensiveLayoutValidator()
    validation_result = await validator.validate_layout(layout_result.layout_data.model_dump())
...
async def _monitor_generation_progress(layout_id: str, user_id: str):
    ...
    logger.info(
        "Layout generation monitoring başlatıldı",
        layout_id=layout_id,
        user_id=user_id
    )
