...
router = APIRouter(
    prefix="/v1/ai", tags=["ai"], dependencies=[Depends(require_api_key)]
)
...
async def create_command(
    body: AICommandCreateRequest,
    db: AsyncSession = Depends(get_db),
    svc: AIService = Depends(get_service),
):
    res = await svc.create_command(
        db, body.command, body.input, body.user_id, body.project_id
    )
...
async def validate_output(body: ValidationRequest):
    ...
    code_errors = validate_building_code(
        body.payload, building_type=body.building_type or "residential"
    )
    errors = schema_errors + geometry_errors + code_errors
    status = (
        "valid"
        if not errors
        else ("requires_review" if len(errors) <= 3 else "rejected")
    )
