...
async def get_slow_queries(
    limit: int = Query(50, ge=1, le=200), _: bool = Depends(require_admin)
):
...
async def start_monitoring(
    interval_seconds: int = Query(60, ge=30, le=3600), _: bool = Depends(require_admin)
):
...
async def run_migrations(
    target_revision: str = "head", _: bool = Depends(require_admin)
):
...
async def rollback_migration(target_revision: str, _: bool = Depends(require_admin)):
...
async def create_backup(
    backup_name: Optional[str] = None, _: bool = Depends(require_admin)
):
...
async def restore_backup(backup_name: str, _: bool = Depends(require_admin)):
