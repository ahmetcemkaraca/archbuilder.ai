...
async def detailed_health_check() -> SystemHealth:
    ...
    performance_metrics = {
        ...
        "avg_response_time_ms": sum(c.response_time_ms for c in components) / len(components) if components else 0,
        "max_response_time_ms": max(c.response_time_ms for c in components) if components else 0
    }
    ...
    try:
        ...
        if cache_stats:
            metrics.update({
                ...
                "redis_total_commands": cache_stats.get("total_commands_processed", 0),
                "redis_ops_per_sec": cache_stats.get("instantaneous_ops_per_sec", 0)
            })
        ...
        if queue_stats:
            metrics.update({
                ...
                "celery_total_scheduled_tasks": queue_stats.get("total_scheduled", 0),
                "celery_total_reserved_tasks": queue_stats.get("total_reserved", 0)
            })
            
            # Per-queue metrics
            for queue_name, queue_data in queue_stats.get("queues", {}).items():
                metrics[f"celery_queue_{queue_name}_active"] = queue_data.get("active", 0)
                metrics[f"celery_queue_{queue_name}_scheduled"] = queue_data.get("scheduled", 0)
                metrics[f"celery_queue_{queue_name}_reserved"] = queue_data.get("reserved", 0)
