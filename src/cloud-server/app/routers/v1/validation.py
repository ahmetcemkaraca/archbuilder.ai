...
async def validate_comprehensive(
    ...
    try:
        ...
        response = JSONResponse(
            content=result,
            headers={"X-Correlation-ID": correlation_id, "X-Request-ID": str(uuid4())},
        )
        ...
        raise HTTPException(status_code=500, detail=f"TR: Validation hatası: {str(e)}")
...
async def validate_schema(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"TR: Schema validation hatası: {str(e)}"
        )
...
async def validate_geometric(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"TR: Geometric validation hatası: {str(e)}"
        )
...
async def validate_code(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"TR: Code validation hatası: {str(e)}"
        )
...
async def analyze_project(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"TR: Proje analizi hatası: {str(e)}"
        )


@router.get("/results/{validation_id}")
async def get_validation_result(
    request: Request, validation_id: str, db: AsyncSession = Depends(get_db)
) -> JSONResponse:
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=404, detail="TR: Validation sonucu bulunamadı"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"TR: Validation sonucu getirme hatası: {str(e)}"
        )
...
async def list_available_schemas(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"TR: Schema listesi hatası: {str(e)}"
        )
