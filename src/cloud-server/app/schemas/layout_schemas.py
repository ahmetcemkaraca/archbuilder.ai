...
class Room(BaseModel):
    ...
    min_height: Optional[float] = Field(
        2.4, ge=2.2, description="Minimum tavan yüksekliği (m)"
    )
    ...
    privacy_level: int = Field(
        1, ge=1, le=3, description="Mahremiyet seviyesi (1=açık, 3=özel)"
    )
    adjacent_rooms: List[str] = Field(
        default_factory=list, description="Bitişik olması gereken odalar"
    )
...
class BuildingRequirements(BaseModel):
    ...
    plot_width: Optional[float] = Field(
        None, ge=5.0, description="Parsel genişliği (m)"
    )
    plot_depth: Optional[float] = Field(
        None, ge=5.0, description="Parsel derinliği (m)"
    )
    ...
    constraints: List[str] = Field(
        default_factory=list, description="Ek tasarım kısıtlamaları"
    )
...
class LayoutGenerationRequest(BaseModel):
    ...
    request_id: str = Field(
        default_factory=lambda: str(uuid4()), description="Talep ID"
    )
    room_program: RoomProgram = Field(..., description="Oda programı")
    building_requirements: BuildingRequirements = Field(
        ..., description="Bina gereksinimleri"
    )
    user_preferences: Dict[str, Any] = Field(
        default_factory=dict, description="Kullanıcı tercihleri"
    )
    reference_layouts: List[str] = Field(
        default_factory=list, description="Referans layout ID'leri"
    )
    priority: int = Field(
        1, ge=1, le=3, description="İşlem önceliği (1=yüksek, 3=düşük)"
    )
...
class ValidationResult(BaseModel):
    ...
    warnings: List[ValidationError] = Field(
        default_factory=list, description="Uyarılar"
    )
    ...
    validated_at: datetime = Field(
        default_factory=datetime.utcnow, description="Validation zamanı"
    )
...
class LayoutResult(BaseModel):
    ...
    validation_result: Optional[ValidationResult] = Field(
        None, description="Validation sonucu"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "model_used": {"provider": "openai", "model": "gpt-4.1"},
...
class ReviewFeedback(BaseModel):
    ...
    corrections: List[Dict[str, Any]] = Field(
        default_factory=list, description="Düzeltmeler"
    )
    notes: str = Field("", description="Mimar notları")
    reviewer_id: str = Field(..., description="İnceleyen kullanıcı ID")
    reviewed_at: datetime = Field(
        default_factory=datetime.utcnow, description="İnceleme zamanı"
    )
...
class ReviewItem(BaseModel):
    ...
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Oluşturma zamanı"
    )
