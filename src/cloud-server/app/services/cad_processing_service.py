...
class CADProcessingService:
    ...
    async def process_cad_file(
        self, file_data: bytes, filename: str, target_format: Optional[CADFormat] = None
    ) -> ProcessingResult:
        """
        ...
        try:
            ...
            if detected_format == CADFormat.DXF:
                ...
                elements, metadata = await self._process_3d_file(
                    file_data, detected_format
                )
            else:
    ...
    async def _process_3d_file(
        self, file_data: bytes, format: CADFormat
    ) -> Tuple[List[CADElement], Dict[str, Any]]:
        ...
        try:
            ...
            elements = [
                CADElement(
                    ...
                    geometry={"type": "mesh", "bounds": metadata["bounding_box"]},
                )
            ]
        ...
        try:
            ...
            for door in doors:
                if door.wall_index < len(walls):
                    ...
                    if wall_length > 0:
                        unit_vec = (
                            wall_vec[0] / wall_length,
                            wall_vec[1] / wall_length,
                        )
                        door_pos = (
