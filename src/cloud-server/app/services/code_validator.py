...
class CodeValidator:
    ...
    async def validate(
        self, payload: Dict[str, Any], building_type: str, region: str = "TR"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if region not in self._building_codes:
                validation_results["errors"].append(
                    f"TR: Desteklenmeyen bölge: {region}"
                )
            ...
            if building_type not in self._building_codes[region]:
                validation_results["errors"].append(
                    f"TR: Desteklenmeyen bina tipi: {building_type}"
                )
                validation_results["valid"] = False
                return validation_results
            ...
            validation_results["code_compliance"] = {
                ...
                "requirements_met": self._count_requirements_met(
                    validation_results["validations"]
                ),
            }
    ...
    async def _validate_room_requirements(
        self, rooms: List[Dict[str, Any]], code_requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for required_room in required_rooms:
            if required_room not in room_types:
                validation["errors"].append(
                    f"TR: Gerekli oda tipi bulunamadı: {required_room}"
                )
                validation["valid"] = False

        # TR: Her oda için alan kontrolü
        for i, room in enumerate(rooms):
            room_validation = await self._validate_room_compliance(
                room, code_requirements, i
            )
    ...
    async def _validate_room_compliance(
        self, room: Dict[str, Any], code_requirements: Dict[str, Any], room_index: int
    ) -> Dict[str, Any]:
    ...
    async def _validate_height_requirements(
        self, dimensions: Dict[str, Any], code_requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _validate_fire_safety(
        self, payload: Dict[str, Any], code_requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if "doors" in payload:
            ...
            if narrow_doors:
                accessibility_checks.append(
                    "TR: Bazı kapılar tekerlekli sandalye için çok dar"
                )
    ...
    def _calculate_compliance_score(self, validations: Dict[str, Any]) -> float:
        ...
        valid_count = sum(
            1 for validation in validations.values() if validation.get("valid", False)
        )
