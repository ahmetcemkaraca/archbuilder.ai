...
class ComprehensiveLayoutValidator:
    ...
    async def validate_layout(self, layout_data: Dict[str, Any]) -> Any:  # ValidationResult
        ...
        try:
            ...
            spatial_errors, spatial_warnings = await self._validate_spatial_constraints(layout)
            ...
            safety_errors, safety_warnings = await self._validate_safety_requirements(layout)
            ...
            confidence = self._calculate_confidence_score(layout, all_errors, all_warnings)
            compliance_score = self._calculate_compliance_score(all_errors, all_warnings)
            
            # Determine if human review is required
            requires_review = self._requires_human_review(status, confidence, all_errors)
    ...
    async def _validate_geometry(self, layout: LayoutData) -> Tuple[List[ValidationError], List[ValidationError]]:
        ...
        for i, wall in enumerate(layout.walls):
            ...
            for j, other_wall in enumerate(layout.walls[i+1:], i+1):
                if self.geometry_utils.lines_overlap(
                    (wall.start, wall.end),
                    (other_wall.start, other_wall.end)
                ):
                    errors.append(ValidationError(
                        code="WALLS_OVERLAP",
        ...
        for i, door in enumerate(layout.doors):
            ...
            if door.position + door.width/2 > wall_length or door.position - door.width/2 < 0:
                errors.append(ValidationError(
            ...
            if door.width < self.min_door_width or door.width > self.max_door_width:
                severity = ErrorSeverity.ERROR if door.width < 800 else ErrorSeverity.WARNING
                errors.append(ValidationError(
        ...
        for i, window in enumerate(layout.windows):
            ...
            if window.position + window.width/2 > wall_length or window.position - window.width/2 < 0:
                errors.append(ValidationError(
    ...
    async def _validate_spatial_constraints(self, layout: LayoutData) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    async def _validate_building_codes(self, layout: LayoutData) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    async def _validate_safety_requirements(self, layout: LayoutData) -> Tuple[List[ValidationError], List[ValidationError]]:
        ...
        window_ratio = total_window_area / total_floor_area if total_floor_area > 0 else 0
    ...
    async def _validate_accessibility(self, layout: LayoutData) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    def _determine_validation_status(
        ...
        critical_errors = [e for e in errors if e.severity == ErrorSeverity.CRITICAL.value]
    ...
    def _calculate_confidence_score(
        ...
        critical_penalty = len([e for e in errors if e.severity == ErrorSeverity.CRITICAL.value]) * 0.3
        error_penalty = len([e for e in errors if e.severity == ErrorSeverity.ERROR.value]) * 0.1
        warning_penalty = len(warnings) * 0.05
        
        adjusted_confidence = max(0.0, base_confidence - critical_penalty - error_penalty - warning_penalty)
    ...
    def _calculate_compliance_score(
        ...
        # Weight by severity
        critical_weight = len([e for e in errors if e.severity == ErrorSeverity.CRITICAL.value]) * 3
        error_weight = len([e for e in errors if e.severity == ErrorSeverity.ERROR.value]) * 2
