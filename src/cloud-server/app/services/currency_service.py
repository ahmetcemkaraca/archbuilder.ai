...
class CurrencyService:
    ...
    def _load_currency_configurations(self) -> None:
        ...
        try:
            ...
            logger.info(f"TR: {len(self._currency_configs)} para birimi yapılandırması yüklendi")
    ...
    def format_currency(self, amount: Union[float, Decimal], 
                       currency_code: str,
        ...
        try:
            ...
            if decimal_places > 0:
                ...
                amount_str = f"{int(amount_decimal):,}".replace(",", thousands_separator)
    ...
    def convert_currency(self, amount: Union[float, Decimal], 
        ...
        try:
            ...
            logger.warning(f"TR: {from_currency} -> {to_currency} dönüşümü için kur bulunamadı")
            return None
    ...
    def get_supported_currencies(self, region: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def calculate_subscription_price(self, base_usd_price: Union[float, Decimal], 
                                   target_currency: str,
                                   region: Optional[str] = None) -> Dict[str, Any]:
        """TR: Subscription fiyatını hedef para birimine çevir"""
        try:
            converted_amount = self.convert_currency(base_usd_price, "USD", target_currency)
            ...
            formatted_price = self.format_currency(converted_amount, target_currency, region)
            ...
            return {
                ...
                "last_updated": self._last_rate_update.isoformat() if self._last_rate_update else None
            }
    ...
    async def update_exchange_rates(self, external_api_data: Optional[Dict[str, Dict[str, float]]] = None) -> bool:
    ...
    def validate_currency_for_region(self, currency_code: str, region: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self.get_supported_currencies(region) if not is_valid else []
        }
