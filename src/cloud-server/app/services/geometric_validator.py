...
class GeometricValidator:
    ...
    async def validate(
        self, payload: Dict[str, Any], building_type: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "rooms" in payload:
                room_validation = await self._validate_rooms(
                    payload["rooms"], building_type
                )
            ...
            if "dimensions" in payload:
                dimension_validation = await self._validate_dimensions(
                    payload["dimensions"], building_type
                )
            ...
            if "structure" in payload:
                structure_validation = await self._validate_structure(
                    payload["structure"], building_type
                )
            ...
            building_specific_validation = await self._validate_building_specific(
                payload, building_type
            )
            validation_results["validations"][
                "building_specific"
            ] = building_specific_validation
            if not building_specific_validation["valid"]:
                validation_results["valid"] = False
                validation_results["errors"].extend(
                    building_specific_validation["errors"]
                )
    ...
    async def _validate_rooms(
        self, rooms: List[Dict[str, Any]], building_type: str
    ) -> Dict[str, Any]:
        """TR: Oda validasyonu"""
        try:
            validation = {
            ...
            for i, room in enumerate(rooms):
                ...
                if "area" in room:
                    ...
                    validation["total_area"] += float(room["length"]) * float(
                        room["width"]
                    )
            ...
            if building_type == "residential":
                if validation["total_area"] < 30.0:
                    validation["warnings"].append(
                        "TR: Konut için toplam alan çok küçük (30m² altı)"
                    )
                elif validation["total_area"] > 300.0:
                    validation["warnings"].append(
                        "TR: Konut için toplam alan çok büyük (300m² üstü)"
                    )
    ...
    async def _validate_single_room(
        self, room: Dict[str, Any], room_index: int
    ) -> Dict[str, Any]:
        ...
        if area is not None:
            if area < self._min_room_area:
                validation["errors"].append(
                    f"TR: Oda {room_index}: Alan çok küçük ({area}m² < {self._min_room_area}m²)"
                )
                validation["valid"] = False
            elif area > self._max_room_area:
                validation["warnings"].append(
                    f"TR: Oda {room_index}: Alan çok büyük ({area}m² > {self._max_room_area}m²)"
                )
        ...
        if "length" in room and "width" in room:
            ...
            if length <= 0 or width <= 0:
                validation["errors"].append(
                    f"TR: Oda {room_index}: Geçersiz boyutlar (uzunluk: {length}, genişlik: {width})"
                )
                validation["valid"] = False
            ...
            if length > 0 and width > 0:
                ...
                if ratio > 3.0:
                    validation["warnings"].append(
                        f"TR: Oda {room_index}: En-boy oranı çok yüksek ({ratio:.1f})"
                    )
        ...
        if "height" in room:
            ...
            if height < self._min_ceiling_height:
                validation["errors"].append(
                    f"TR: Oda {room_index}: Tavan yüksekliği çok düşük ({height}m < {self._min_ceiling_height}m)"
                )
                validation["valid"] = False
            elif height > self._max_ceiling_height:
                validation["warnings"].append(
                    f"TR: Oda {room_index}: Tavan yüksekliği çok yüksek ({height}m > {self._max_ceiling_height}m)"
                )

        return validation

    async def _validate_dimensions(
        self, dimensions: Dict[str, Any], building_type: str
    ) -> Dict[str, Any]:
        ...
        if "height" in dimensions:
            ...
            if height < self._min_ceiling_height:
                validation["errors"].append(
                    f"TR: Yükseklik çok düşük ({height}m < {self._min_ceiling_height}m)"
                )
                validation["valid"] = False
    ...
    async def _validate_structure(
        self, structure: Dict[str, Any], building_type: str
    ) -> Dict[str, Any]:
        """TR: Yapı validasyonu"""
        validation = {"valid": True, "errors": [], "warnings": []}
        ...
        if "type" in structure:
            ...
            if structure["type"] not in valid_types:
                validation["errors"].append(
                    f"TR: Geçersiz yapı tipi: {structure['type']}"
                )
                validation["valid"] = False
    ...
    async def _validate_building_specific(
        ...
        if building_type == "residential":
            ...
            if "rooms" in payload:
                rooms = payload["rooms"]
                bedroom_count = sum(
                    1 for room in rooms if room.get("type") == "bedroom"
                )
                if bedroom_count == 0:
                    validation["warnings"].append(
                        "TR: Konut için yatak odası bulunamadı"
                    )

                bathroom_count = sum(
                    1 for room in rooms if room.get("type") == "bathroom"
                )
                if bathroom_count == 0:
            ...
            if "rooms" in payload:
                ...
                if office_count == 0:
                    validation["warnings"].append(
                        "TR: Ticari yapı için ofis alanı bulunamadı"
                    )
