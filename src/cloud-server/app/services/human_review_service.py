...
class HumanReviewService:
    ...
    async def submit_for_review(
        ...
        if auto_actions.get("auto_approve", False):
            ...
            review_item.feedback_history.append(ReviewFeedback(
    ...
    async def get_reviewer_dashboard(self, reviewer_id: str) -> Dict[str, Any]:
        ...
        pending_items = [item for item in assigned_items if item.status == ReviewStatus.IN_REVIEW]
        completed_items = [item for item in assigned_items if item.status in [
            ReviewStatus.APPROVED, ReviewStatus.REJECTED, ReviewStatus.COMPLETED
        ]]
        ...
        if completed_items:
            ...
            for item in completed_items:
                if item.feedback_history:
                    ratings.extend([f.rating for f in item.feedback_history if f.reviewer_id == reviewer_id])
            if ratings:
    ...
    async def get_system_stats(self) -> ReviewStats:
        ...
        pending_items = [item for item in all_items if item.status == ReviewStatus.PENDING]
        completed_items = [item for item in all_items if item.status in [
            ReviewStatus.APPROVED, ReviewStatus.REJECTED, ReviewStatus.COMPLETED
        ]]
        
        # Calculate approval rate
        approved_items = [item for item in completed_items if item.status == ReviewStatus.APPROVED]
        approval_rate = len(approved_items) / len(completed_items) if completed_items else 0
        ...
        for reviewer_id in self.reviewer_workload.keys():
            reviewer_items = [item for item in all_items if item.assigned_reviewer == reviewer_id]
            reviewer_performance[reviewer_id] = {
                "total_reviews": len(reviewer_items),
                "current_workload": self.reviewer_workload[reviewer_id],
                "avg_rating": self._calculate_reviewer_avg_rating(reviewer_id, reviewer_items)
            }
    ...
    async def _analyze_review_requirements(
        ...
        if (ai_confidence and ai_confidence >= self.auto_approve_threshold and
            not has_errors and
            all(vr.status.value == "valid" for vr in validation_results)):
    ...
    async def _assign_reviewer(self, review_item: ReviewItem) -> Optional[str]:
        ...
        for reviewer_id in available_reviewers:
            ...
            if current_workload < min_workload and current_workload < self.max_reviewer_workload:
                min_workload = current_workload
                best_reviewer = reviewer_id
        
        if best_reviewer:
            self.reviewer_workload[best_reviewer] = self.reviewer_workload.get(best_reviewer, 0) + 1
    ...
    async def _calculate_complexity_score(
        ...
        special_requirements = sum(1 for req in special_reqs if req in [
            "accessible_design", "fire_safety", "seismic", "energy_efficient"
        ])
    ...
    def _get_recent_activity(self, reviewer_id: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        for item in reviewer_items[:limit]:
            ...
            if item.feedback_history:
                reviewer_feedbacks = [f for f in item.feedback_history if f.reviewer_id == reviewer_id]
                if reviewer_feedbacks:
                    latest_feedback = max(reviewer_feedbacks, key=lambda x: x.created_at)
            ...
            activities.append({
                ...
                "approval_status": latest_feedback.approval_status if latest_feedback else None
            })
        
        return activities

    def _calculate_reviewer_avg_rating(self, reviewer_id: str, items: List[ReviewItem]) -> float:
    ...
    async def cleanup_old_reviews(self, days: int = 30) -> int:
        ...
        items_to_remove = [
            item_id for item_id, item in self.review_queue.items()
            if item.status in [ReviewStatus.APPROVED, ReviewStatus.COMPLETED, ReviewStatus.REJECTED]
