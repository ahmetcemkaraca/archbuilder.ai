...
class LayoutGenerationService:
    ...
    async def _generate_ai_layout(
        ...
        try:
            ...
            layout_data["model_used"] = f"{model_config['provider']}:{model_config['model']}"
            return layout_data
    ...
    def _select_optimal_model(self, request: LayoutGenerationRequest) -> Dict[str, Any]:
        ...
        complexity = "simple"
        if len(request.room_program.rooms) > 8 or len(request.building_requirements.constraints) > 5:
            complexity = "high"
    ...
    async def _generate_fallback_layout(
        ...
        logger.info(
            "Fallback layout generated successfully",
            layout_id=layout_result.layout_id
        )
    ...
    async def _validate_input(self, request: LayoutGenerationRequest):
        ...
        for room in request.room_program.rooms:
            if room.area < 5.0:
                errors.append(f"Oda '{room.name}' minimum alan gereksinimini (5m²) karşılamıyor")
