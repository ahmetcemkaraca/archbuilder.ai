...
class LayoutIntegrationService:
    ...
    async def generate_layout_workflow(
        ...
        try:
            ...
            workflow_state["stages"]["generation"] = {"status": "processing", "started_at": datetime.now()}
            ...
            workflow_state["stages"]["validation"] = {"status": "processing", "started_at": datetime.now()}
            
            validation_results = []
            if layout_result.get("layout_data"):
                validation_result = await self.validator.validate_layout(layout_result["layout_data"])
            ...
            needs_review = self._determine_review_requirement(layout_result, validation_results)
            
            if needs_review:
                logger.info("Stage 3: Human review assignment", workflow_id=workflow_id)
                workflow_state["stages"]["review"] = {"status": "processing", "started_at": datetime.now()}
            ...
            workflow_state["status"] = "completed" if not needs_review else "pending_review"
            ...
            result = {
                ...
                "review_id": workflow_state["stages"].get("review", {}).get("review_id"),
                "processing_time_ms": int(
                    (workflow_state.get("completed_at", datetime.now()) - workflow_state["started_at"]).total_seconds() * 1000
                )
            }
            ...
            return {
                ...
                "error": str(e),
                "processing_time_ms": int(
                    (datetime.now() - workflow_state["started_at"]).total_seconds() * 1000
                )
            }
    ...
    async def process_cad_file_workflow(
        ...
        file_data: bytes,
        filename: str,
        analysis_type: str = "existing_project"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if analysis_type == "existing_project":
                ...
                analysis_result = await self.cad_service.analyze_existing_project(file_data, filename)
                ...
                processing_result = await self.cad_service.process_cad_file(file_data, filename)
                ...
                return {
                    ...
                    "analysis_type": analysis_type,
                    "filename": filename,
                    "processing_result": processing_result.__dict__ if hasattr(processing_result, '__dict__') else processing_result
                }
    ...
    async def get_review_queue_dashboard(self, reviewer_id: Optional[str] = None) -> Dict[str, Any]:
        """Get review dashboard data"""
        
        try:
            if reviewer_id:
                # Get specific reviewer dashboard
                dashboard = await self.review_service.get_reviewer_dashboard(reviewer_id)
            else:
                ...
                dashboard = {
                    "system_stats": stats.__dict__ if hasattr(stats, '__dict__') else stats,
                    "recent_queue": [item.__dict__ if hasattr(item, '__dict__') else item for item in queue]
                }
    ...
    async def submit_review_feedback(
        ...
        try:
            ...
            return {
                "success": success,
                "review_id": review_id,
                "message": "Feedback başarıyla gönderildi" if success else "Feedback gönderilemedi"
            }
    ...
    async def export_layout_to_cad(
        ...
        try:
            ...
            if target_format.lower() == "dxf":
                cad_data = await self.cad_service.convert_layout_to_dxf(walls, doors, windows)
    ...
    async def cleanup_old_workflows(self, hours: int = 24) -> int:
        ...
        workflows_to_remove = [
            wf_id for wf_id, wf_data in self.active_generations.items()
