...
class LicenseService:
    ...
    async def validate_license(
        self, user_id: str, license_key: Optional[str] = None
    ) -> LicenseInfo:
        """
    ...
    async def _get_license_from_db(
        self, user_id: str, license_key: Optional[str] = None
    ) -> LicenseInfo:
    ...
    async def _validate_subscription_info(
    ...
    async def _get_usage_limits(self, user_id: str, usage_type: UsageType) -> Dict:
        ...
        return {"limit": limit, "reset_period": reset_period, "reset_date": reset_date}
    ...
    async def _update_usage(
        self, usage_key: str, usage: int, reset_period: str
    ) -> None:
    ...
    async def _cache_license(self, cache_key: str, license_info: LicenseInfo) -> None:
        ...
        try:
            ...
            self.redis_client.setex(
                cache_key, self.license_cache_ttl, json.dumps(license_dict)
            )
        except Exception as e:
            logger.error(f"Failed to cache license: {str(e)}")

    async def _get_cached_subscription(
        self, cache_key: str
    ...
    async def _cache_subscription(
        self, cache_key: str, subscription_info: SubscriptionInfo
    ) -> None:
        """Cache subscription information"""
        try:
            subscription_dict = subscription_info.dict()
            self.redis_client.setex(
                cache_key, self.license_cache_ttl, json.dumps(subscription_dict)
            )
        except Exception as e:
