...
class RAGMetricsService:
        ...
        try:
            ...
            scenarios = [
                RAGTestScenario(
                    ...
                    expected_documents=[
                        "revit_wall_creation.pdf",
                        "basic_revit_guide.pdf",
                    ],
                ...
                RAGTestScenario(
                    ...
                    expected_documents=[
                        "turkish_building_codes.pdf",
                        "residential_standards.pdf",
                    ],
                ...
                RAGTestScenario(
                    ...
                    expected_documents=[
                        "structural_systems.pdf",
                        "multi_story_design.pdf",
                        "engineering_analysis.pdf",
                    ],
                ...
                RAGTestScenario(
                    ...
                    expected_documents=[
                        "concrete_classes.pdf",
                        "residential_materials.pdf",
                    ],
    ...
    async def calculate_precision_recall(
        self, retrieved_documents: List[str], relevant_documents: List[str]
    ) -> Tuple[float, float]:
        ...
        try:
            ...
            precision = (
                len(relevant_retrieved) / len(retrieved_documents)
            ...
            recall = (
                len(relevant_retrieved) / len(relevant_documents)
    ...
    async def run_test_scenario(
        self, scenario: RAGTestScenario, rag_client: Any  # TR: RAG client interface
    ) -> RAGMetrics:
        ...
        try:
            ...
            rag_response = await rag_client.query(
                query=scenario.query, context=scenario.context, max_results=10
            )
            ...
            retrieved_docs = [
                doc.get("id", "") for doc in rag_response.get("documents", [])
            ]
            ...
            answer_accuracy = (
                1.0
                if scenario.expected_answer.lower() in predicted_answer.lower()
                else 0.0
            )
    ...
    async def run_comprehensive_tests(
        self, rag_client: Any, difficulty_filter: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"TR: Test çalıştırma hatası: {str(e)}"}

    def _calculate_aggregate_metrics(
        self, metrics_list: List[RAGMetrics]
    ) -> Dict[str, Any]:
        """TR: Aggregate metrics hesapla"""
        if not metrics_list:
            return {}
        ...
        return {
            "precision": {
                ...
                "std": (
                    statistics.stdev([m.precision for m in metrics_list])
            ...
            "recall": {
                ...
                "std": (
                    statistics.stdev([m.recall for m in metrics_list])
                    if len(metrics_list) > 1
                    else 0.0
                ),
            },
            "f1_score": {
                ...
                "std": (
                    statistics.stdev([m.f1_score for m in metrics_list])
                    if len(metrics_list) > 1
                    else 0.0
                ),
            },
            "accuracy": {
                ...
                "std": (
                    statistics.stdev([m.accuracy for m in metrics_list])
                    if len(metrics_list) > 1
                    else 0.0
                ),
            },
            "response_time": {
                ...
                "std": (
                    statistics.stdev([m.response_time for m in metrics_list])
                    if len(metrics_list) > 1
                    else 0.0
                ),
            },
            "confidence_score": {
                ...
                "std": (
                    statistics.stdev([m.confidence_score for m in metrics_list])
                    if len(metrics_list) > 1
                    else 0.0
                ),
            },
        ...
        for difficulty, group_metrics in difficulty_groups.items():
            analysis[difficulty] = {
                ...
                "avg_response_time": statistics.mean(
                    [m.response_time for m in group_metrics]
                ),
            }
