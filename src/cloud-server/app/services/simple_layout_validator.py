...
class SimpleValidationError:
    ...
    def __init__(self, code: str, message: str, severity: str = "error", location: str = None, suggestion: str = None):
        self.code = code
...
class SimpleLayoutValidator:
    ...
    async def validate_layout(self, layout_data: Dict[str, Any]) -> SimpleValidationResult:
        """
        ...
        try:
            ...
            result.confidence = max(0.0, 1.0 - (len(result.errors) * 0.2) - (len(result.warnings) * 0.1))
            
            logger.info(
                "Layout validation tamamlandı - Valid: %s, Errors: %d, Warnings: %d, Confidence: %.2f",
                result.is_valid, len(result.errors), len(result.warnings), result.confidence
            )
            
        except (KeyError, ValueError, TypeError) as e:
            logger.error("Layout validation hatası: %s", str(e))
            result.errors.append(SimpleValidationError(
    ...
    async def _validate_walls(self, walls: List[Dict[str, Any]], result: SimpleValidationResult):
        """Wall validation"""
        
        if not walls:
            result.warnings.append(SimpleValidationError(
                code="NO_WALLS",
    ...
    async def _validate_doors(self, doors: List[Dict[str, Any]], walls: List[Dict[str, Any]], result: SimpleValidationResult):
        """Door validation"""
    ...
    async def _validate_windows(self, windows: List[Dict[str, Any]], walls: List[Dict[str, Any]], result: SimpleValidationResult):
        """Window validation"""
    ...
    async def _validate_rooms(self, rooms: List[Dict[str, Any]], result: SimpleValidationResult):
        """Room validation"""
