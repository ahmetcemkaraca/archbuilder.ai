...
class ValidationService:
    ...
    async def validate_json_schema(self, data: Any, schema_name: str) -> Dict[str, Any]:
    ...
    async def validate_geometric(
        ...
        try:
            ...
            return {"success": result.get("valid", False), "data": result}
        except Exception as e:
    ...
    async def validate_comprehensive(
        ...
        try:
            ...
            if "schema" in validation_types:
                schema_result = await self.validate_json_schema(
    ...
    async def analyze_project(
        ...
        try:
            ...
            schema_result = await self.validate_json_schema(
                request_data, "ProjectAnalysisRequest"
            )
            if not schema_result["success"]:
                return {
                    "success": False,
                    "data": {
                        "error": "TR: Geçersiz istek formatı",
            ...
            if analysis_type in ["geometry", "both"]:
                try:
                    geometry_result = await self._geometric_validator.analyze_project(
                        project_id, options
                    )
            ...
            if analysis_type in ["code", "both"]:
                try:
                    code_result = await self._code_validator.analyze_project(
                        project_id, options
                    )
            ...
            if results["confidence_score"] < 0.7:
                results["recommendations"].append(
                    "TR: Proje analizi eksik, daha detaylı inceleme önerilir"
                )
            ...
            response_schema_result = await self.validate_json_schema(
                response_data, "ProjectAnalysisResponse"
            )
            if not response_schema_result["success"]:
                logger.warning(
                    f"TR: Response schema validation başarısız: {response_schema_result['errors']}"
                )
