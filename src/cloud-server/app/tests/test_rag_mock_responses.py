...
class TestRAGMockResponses:
    ...
    async def test_hybrid_search_mock_success(
    ...
    async def test_ensure_dataset_mock_success(
        self, app: FastAPI, mock_ragflow_responses
    ):
    ...
    async def test_upload_parse_async_mock_success(
        self, app: FastAPI, mock_ragflow_responses
    ):
    ...
    async def test_job_status_mock_completed(
        self, app: FastAPI, mock_ragflow_responses
    ):
    ...
    async def test_job_status_mock_processing(
        self, app: FastAPI, mock_ragflow_responses
    ):
    ...
    async def test_rag_query_mock_with_filters(
        self, app: FastAPI, mock_ragflow_responses
    ):
        ...
        filtered_response = {
            ...
            "data": {
                ...
                "total_results": 1,
                "filters_applied": {"document_types": ["pdf"], "languages": ["en"]},
            },
        }
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.post(
                    ...
                    json={
                        ...
                        "document_types": ["pdf"],
                        "languages": ["en"],
                        "filters": {"date_range": "2024-01-01,2025-01-01"},
                    },
                    headers={"X-API-Key": "test-api-key"},
                )
    ...
    async def test_rag_query_mock_timeout(self, app: FastAPI):
        ...
        with patch('app.services.rag_service.RAGService') as mock_service_class:
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.post(
                    "/v1/rag/query",
                    json={"query": "timeout test", "dataset_ids": ["dataset_123"]},
                    headers={"X-API-Key": "test-api-key"},
                )
    ...
    async def test_rag_query_mock_unauthorized(self, app: FastAPI):
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            ...
            response = await ac.post(
                "/v1/rag/query",
                json={"query": "unauthorized test", "dataset_ids": ["dataset_123"]},
            )
    ...
    async def test_rag_query_mock_rate_limit(self, app: FastAPI):
        ...
        with patch('app.services.rag_service.RAGService') as mock_service_class:
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.post(
                    "/v1/rag/query",
                    json={"query": "rate limit test", "dataset_ids": ["dataset_123"]},
                    headers={"X-API-Key": "test-api-key"},
                )
    ...
    async def test_rag_query_mock_performance(self, app: FastAPI):
        ...
        with patch('app.services.rag_service.RAGService') as mock_service_class:
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                ...
                response = await ac.post(
                    "/v1/rag/query",
                    json={"query": "performance test", "dataset_ids": ["dataset_123"]},
                    headers={"X-API-Key": "test-api-key"},
                )
