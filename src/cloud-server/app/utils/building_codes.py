...
class TurkishBuildingCodeValidator:
    ...
    async def validate_layout(
        ...
        try:
            ...
            # 5. Engelli erişimi
            access_errors, access_warnings = (
                await self._validate_accessibility_compliance(layout)
            )
            errors.extend(access_errors)
            warnings.extend(access_warnings)

            # 6. Havalandırma gereksinimleri
            ventilation_errors, ventilation_warnings = await self._validate_ventilation(
                layout
            )
    ...
    async def _validate_room_areas(
        self, layout: LayoutData
    ...
    async def _validate_door_requirements(
        self, layout: LayoutData
    ) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    async def _validate_natural_lighting(
        self, layout: LayoutData
    ) -> Tuple[List[ValidationError], List[ValidationError]]:
        ...
        for room in layout.rooms:
            ...
            if room_function in self.natural_light_ratios:
                ...
                actual_ratio = (
                    room_window_area / (room.area * 1_000_000) if room.area > 0 else 0
                )  # Convert m² to mm²

                if actual_ratio < required_ratio:
                    shortage_pct = (
                        (required_ratio - actual_ratio) / required_ratio * 100
                    )
    ...
    async def _validate_fire_safety(
        ...
        for i, door in enumerate(layout.doors):
            if door.wall_index < len(layout.walls):
                ...
                if (
                    wall.type == "exterior"
                    and door.width >= self.door_widths["fire_exit"]["min"]
                ):
    ...
    async def _validate_accessibility_compliance(
        ...
        accessible_entrance = any(
            door.width >= wheelchair_min_door for door in main_doors
        )
        ...
        # İç mekan erişimi
        narrow_doors = [
            i for i, door in enumerate(layout.doors) if door.width < wheelchair_min_door
        ]
    ...
    async def _validate_ventilation(
        self, layout: LayoutData
    ) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    def _classify_room_function(self, room_name: str) -> RoomFunction:
        ...
        if any(keyword in name_lower for keyword in ["salon", "oturma", "living"]):
            ...
            keyword in name_lower for keyword in ["banyo", "wc", "tuvalet", "bathroom"]
        ):
    ...
    def _calculate_room_window_area(
        self, room: RoomBoundary, layout: LayoutData
    ) -> float:
    ...
    def _identify_escape_routes(self, layout: LayoutData) -> List[Dict[str, Any]]:
        ...
        for corridor in corridors:
            ...
            estimated_width = (
                corridor.area * 1000
            ) ** 0.5 * 1000  # Rough estimation in mm
            escape_routes.append(
                {"name": corridor.name, "width": estimated_width, "area": corridor.area}
            )
