...
class GeometryUtils:
    ...
    def lines_overlap(
        ...
        if cross_product < 0.01:  # Nearly parallel
            ...
            dist_to_line = self._point_to_line_distance(
                Point2D(x=p2_start[0], y=p2_start[1]),
                line1[0], line1[1]
            )
        ...
        if intersection is not None:
            ...
            if (self._point_on_line_segment(intersection, p1_start, p1_end, tolerance) and
                self._point_on_line_segment(intersection, p2_start, p2_end, tolerance)):
                return True
    ...
    def _line_segments_overlap(
        self, 
        p1_start: np.ndarray, p1_end: np.ndarray,
    ...
    def is_rectangle_valid(self, corners: List[Point2D], tolerance: float = 10.0) -> bool:
        """
