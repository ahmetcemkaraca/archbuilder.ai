...
class TestAnalyzeProject:
    ...
    async def test_analyze_project_both_types(self, validation_service, mock_db):
        ...
        assert result["data"]["code_analysis"]["valid"] is True
        assert (
            result["data"]["confidence_score"] == 1.0
        )  # TR: Her iki analiz de başarılı
        assert len(result["data"]["recommendations"]) > 0
    ...
    async def test_analyze_project_geometric_failure(self, validation_service, mock_db):
        ...
        assert result["data"]["code_analysis"]["valid"] is True
        assert (
            result["data"]["confidence_score"] == 0.5
        )  # TR: Sadece kod analizi başarılı
        assert "TR: Geometrik validasyon başarısız" in result["data"]["recommendations"]

    @pytest.mark.asyncio
    async def test_analyze_project_invalid_request_schema(
    ...
    @pytest.mark.asyncio
    async def test_analyze_project_geometric_exception(
        self, validation_service, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_project_low_confidence_recommendations(
        self, validation_service, mock_db
    ):
        ...
        assert result["success"] is True
        assert (
            result["data"]["confidence_score"] == 0.0
        )  # TR: Her iki analiz de başarısız
